...
for i = 1:size(modelMatrix,1)
    ...
    stateTransitions(i,:) = sum(asymmetricProbabilities(thisStates,:), 1);
    ...
    for l = 1:maxClusters
        ...
        for i = 1:length(plotLoops)
            ...
            for k = 1:length(allClusteredLoops)
                ...
                for j = 1:length(drawPath)
                    allClusterIndicies = unique([allClusterIndicies; find(clusterIDs == drawPath(j))]);
                end
            end
        end
